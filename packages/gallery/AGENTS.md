...
```tsx
<Gallery files={galleryStoryFiles} />
```
...
`Gallery` is exported by `@chayns-components/gallery` and should be imported from the public package
entry point.
...
```tsx
<Gallery files={galleryStoryFiles.slice(0, 3)} isEditMode maxFiles={6} />
```

#### Wrapper Read Only Square

```tsx
<Gallery files={galleryViewerSquareFiles} viewMode={GalleryViewMode.SQUARE} />
```
...
`GalleryEditor` is exported by `@chayns-components/gallery` and should be imported from the public
package entry point.
...
`GalleryViewer` is exported by `@chayns-components/gallery` and should be imported from the public
package entry point.
...
```tsx
<GalleryViewer files={galleryStoryFiles} viewMode={GalleryViewMode.GRID} />
```

#### Square Mode

```tsx
<GalleryViewer files={galleryViewerSquareFiles} viewMode={GalleryViewMode.SQUARE} />
```
