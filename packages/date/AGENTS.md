...
- `CalendarType` -> `enum CalendarType {
    Single = 'single',
    Multiple = 'multiple',
    Interval = 'interval',
}`
...
- `CustomThumbColors` -> `interface CustomThumbColors {
    /**
...
- `HighlightedDates` -> `interface HighlightedDates {
    dates: Date[];
...
- `HintTextPosition` -> `enum HintTextPosition {
    Top,
    Bottom,
}`
