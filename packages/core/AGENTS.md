...
`Masonry` is exported by `@chayns-components/core` and should be imported from the public package
entry point.
...
```tsx
<Masonry gap={16} columnWidth={160} rowHeight={80} />
```
...
`Skeleton` is exported by `@chayns-components/core` and should be imported from the public package
entry point.
...
```tsx
<Skeleton lines={3} randomWithBounds={[50, 90]} />
```
