...
class OpeningTimes extends Component {
    constructor(props) {
        ...
        this.onValidateStateDebounce = debounce(
            this.onValidateState.bind(this),
            1000
        );
        ...
        for (let i = 0; i < times.length; i += 1) {
            ...
            if (
                current &&
                (current.disabled === null || current.disabled === undefined)
            )
    ...
    onAdd(weekDay) {
        ...
        const foundTimes = newState.times.filter(
    ...
    onRemove(day, span) {
        ...
        const timesOfDay = newState.times
            .filter((time) => time.weekDay === day)
            .filter((time, index) => index !== span);
        const otherTimes = newState.times.filter(
    ...
    onChange(day, index, start, end) {
        ...
        if (onChange) {
            const timesOfDay = newState.times.filter(
                (time) => time.weekDay === day
            );
    ...
    onDayActivation(day, status) {
        ...
        if (status) {
            if (
                newState.times.length ===
                    newState.times.filter((time) => time.disabled).length ||
                newState.times.length === 0
            ) {
    ...
    render() {
        ...
        return (
            <div
                className={classNames(className, 'cc__opening_times', {
                    'cc__opening_times--force-mobile': forceMobile,
                })}
                style={style}
            >
                {!timesValid &&
